  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      tags$div(
        ...
        HTML('<div style="text-align: center; font-size: 25px; font-family: Arial;">Welcome to the data dashboard 
             ...
             about the dataset, you may find it helpful reference <a href="#" download="contextual_factors_codebook.docx">this codebook</a>.</div>')
      )
    ),
    ## (1) Data info-----
    tabPanel("Data info & availability",
             fluidRow(
               ...
               column(3, pickerInput("variable1", label = h4("Variables available for comparison"), 
    ...
    tabPanel("Trends and statistics",
             sidebarPanel(
               tags$div(pickerInput("sex_variable2", label = h4("Sex (male/female) comparison variables"), 
                              names(variable_info[["sex comparison variables"]]),
                              selected = "employed", multiple = TRUE, options = list(`actions-box` = TRUE)),
                        id = "sex_variable2"),
               ...
               pickerInput("state2", label = h4("States"), sort(unique(shiny_data$state_label)),
                              selected = "Pennsylvania (FIPS 42)", multiple = TRUE, options = list(`actions-box` = TRUE)),
               pickerInput("county2", label = h4("Counties"), 
              ...
              sliderInput("year2", h4("Years"), min = 1850, max = max(shiny_data$YEAR), value = c(1850, max(shiny_data$YEAR)), sep=""),
    ...
    tabPanel("Heatmaps",
             fluidRow(
               ...
               column(2, pickerInput("Variable_map_2", label = h4("Variables available for comparison"), 
                                     names(variable_info[["sex comparison variables"]]), 
                                     selected = "employed", multiple = FALSE)),
               ...
               conditionalPanel(
                 ...
                 column(2, pickerInput(inputId = "state_for_county_selection_dropdown", label = h4("State"),
                                       choices = sort(unique(shiny_data$state_label)), selected = "Pennsylvania (42)"))),
               column(2, sliderInput("Year_2", h4("Year"), min = 1850, max = max(shiny_data$YEAR), value = max(shiny_data$YEAR), step = 12, animate = TRUE, sep="")),
    ...
    tabPanel("Inter-state trends",
             fluidRow(
               column(2, pickerInput("sex_variables_ist", label = h4("Sex (male/female) comparison variables"), names(variable_info[["sex comparison variables"]]),
               ...
               column(2, pickerInput("race_variables_ist", label = h4("Race (white/non-white) comparison variables"), names(variable_info[["race comparison variables"]]),
    ...
    tabPanel("Download",
             sidebarPanel(
               ...
               pickerInput("sex_variable5", label = h4("Sex (male/female) comparison variables"), names(variable_info[["sex comparison variables"]]),
                              selected = "employed", multiple = TRUE, options = list(`actions-box` = TRUE)),
               bsTooltip("sex_variable5", "Select variable to examine the ratio of the percentage of males to the percentage of females.", "top"),
               pickerInput("race_variable5", label = h4("Race (white/non-white) comparison variables"), names(variable_info[["race comparison variables"]]),
                              selected = "employed", multiple = TRUE, options = list(`actions-box` = TRUE)),
               ...
               pickerInput("States_table", label = h4("States"), sort(unique(shiny_data$state_name)),
                           selected = "Pennsylvania", multiple = TRUE, options = list(`actions-box` = TRUE)),
               radioButtons("County_table", label = h4("Include county-level data"), choices = c("Yes", "No"), selected = "No"),
               sliderInput("Year_table", h4("Years"), min = 1850, max = max(shiny_data$YEAR), value = c(1850, max(shiny_data$YEAR)), sep=""),
               br(),
...
server <- function(input, output) {
  ...
  observe({
    ...
    updateSelectizeInput("Variable_map_2",
